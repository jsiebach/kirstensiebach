...
@section('body')
    ...
    @markdown($page->intro)
    <h2>Current Team</h2>
    @foreach(\App\Models\TeamMember::where('alumni', false)->ordered()->get() as $i => $teamMember)
        @include('partials.lab-team', compact('i', 'teamMember'))
    @endforeach
    <h2>Lab Alumni</h2>
    @foreach(\App\Models\TeamMember::where('alumni', true)->ordered()->get() as $i => $teamMember)
        @include('partials.lab-team', compact('i', 'teamMember'))
    @endforeach
    <hr>
